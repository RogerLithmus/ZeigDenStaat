...
# -*- coding: utf-8 -*-
"""
maintain_federal_directory.py - Master Maintenance & Enrichment Pipeline for German Federal Entities.

This script consolidates all previous scripts in the scripts/ directory into a single, cohesive,
production-grade tool. It:
1. Loads the master list of the federal entities from resolved_mappings.csv.
2. Systematically searches Wikipedia and scrapes official infoboxes for employee counts (Mitarbeiteranzahl) and budgets (Budget)
   for all 1,077+ federal authorities.
3. Incorporates curated official metrics for major entities.
4. Cleans and normalizes values.
5. Builds and styles a beautiful, premium Excel workbook (Bundesbehörden_Verzeichnis.xlsx) matching the "Midnight Navy & Gold" design system.
"""
...
CSV_PATH = os.path.join(BASE_DIR, "data", "legacy", "resolved_mappings.csv")
OUTPUT_PATH = os.path.join(BASE_DIR, "data", "Bundesbehörden_Verzeichnis.xlsx")
...
DELAY = 0.3  # Polite delay between Wikipedia requests

# ---------------------------------------------------------------------------
# Curated Metrics (Verified Hand-Picked Benchmarks)
# ---------------------------------------------------------------------------
curated_data_raw = {
    2: {
...
# ---------------------------------------------------------------------------
# Wikipedia Data Harvesting & Scraping Logic
# ---------------------------------------------------------------------------
...
def scrape_wikipedia_infobox(url: str) -> dict:
    result = {"Mitarbeiteranzahl": "", "Budget": "", "Website": ""}
    try:
        time.sleep(DELAY)
        ...
        for row in rows:
            ...
            if len(cells) == 2:
                ...
                if any(k in label for k in ["mitarbeiter", "beschäftigte", "bedienstete", "personal", "mitarbeiterzahl"]):
                    result["Mitarbeiteranzahl"] = value
                elif any(k in label for k in ["etat", "haushalt", "budget", "umsatz"]):
                    result["Budget"] = value
                elif any(k in label for k in ["website", "webpräsenz", "webseite", "homepage"]):
                    a_tag = cells[1].find("a", href=True)
                    if a_tag:
                        href = a_tag["href"]
                        if href.startswith("//"):
                            href = "https:" + href
                        result["Website"] = href if href.startswith("http") else value
                    else:
...
def clean_scraped_employees(val: str) -> float | None:
    if not val:
        return None
    val_clean = re.sub(r"\s+", "", val)
    # Match numbers like 3.200, 3200, 3200,5
...
def clean_scraped_budget(val: str) -> float | None:
    if not val:
        return None
    val_clean = val.lower().replace(",", ".")
    # Match numeric budget value
    match = re.search(r"(\d+(?:\.\d+)?)", val_clean)
    if match:
        num = float(match.group(1))
        if "mrd" in val_clean or "milliarde" in val_clean:
            return num * 1000.0
        elif "mio" in val_clean or "million" in val_clean:
            return num
        else:
            # If large figure without suffix
            if num > 1000000:
...
def main():
    print("=" * 80)
    print("ZEIGDENSTAAT - SYSTEMATIC MAINTENANCE & ENRICHMENT PIPELINE")
    print("=" * 80)

    # 1. Resolve curated entities standard mappings
    ...
    for target_name, original_key in name_to_original_key.items():
        ...
        if match:
            ...
            log.info(f"Resolved mapping: '{target_name}' -> ID: {actual_id}")

    # 2. Iterate and enrich all entities
    log.info("Starting systematic online enrichment via Wikipedia OpenSearch & Infobox Scraping...")
    processed_rows = []
    ...
    for idx, row in df.iterrows():
        ...
        if nid in curated_data:
            ...
            if not is_aggregated:
                # 2. Check Wikipedia to systematically fill gaps in Mitarbeiteranzahl & Budget
                # Only run this for authorities/entities that have a high likelihood of a Wiki entry
                wiki_url = find_wikipedia_url(name)
                if wiki_url:
                    info = scrape_wikipedia_infobox(wiki_url)
                    if info.get("Mitarbeiteranzahl"):
                        employees = clean_scraped_employees(info["Mitarbeiteranzahl"])
                    if info.get("Budget"):
                        budget = clean_scraped_budget(info["Budget"])
                    if info.get("Website"):
                        website = info["Website"]
                    
                    if employees or budget:
                        log.info(f"  Enriched from Wikipedia: '{name}' -> Employees: {employees}, Budget: {budget} Mio. €")

                # Default fallback values for non-aggregated entities
                if category == "Bundesunternehmen":
                    ...
                    website = website or "www.service.bund.de"
    ...
    # 5. Save Spreadsheet
    log.info("Saving corrected spreadsheet...")
    wb.save(OUTPUT_PATH)
    log.info(f"Spreadsheet successfully generated and saved at {OUTPUT_PATH} ")

# --- Helper: Extract Headquarters ---
major_cities = [
