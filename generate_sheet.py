...
# Define paths
csv_path = r"C:\Projects\ZeigDenStaat\data\resolved_mappings.csv"
output_path = r"C:\Projects\ZeigDenStaat\Bundesbehörden_Verzeichnis.xlsx"
...
# Curated data for the top 60 entities
curated_data = {
    2: {
    ...
    118: {
        "Budget": 42.0, "Employees": 200, "Headquarters": "Berlin", "LegalForm": "Oberstes Verfassungsorgan-Verwaltung",
...
# Major cities for programmatically matching headquarters
major_cities = [
...
def extract_headquarters(name, classification):
    # Try case-insensitive matching with major cities first
    name_str = str(name)
    for city in major_cities:
        if city.lower() in name_str.lower():
            return city
            
    # Trailing part extraction
    prefixes = [
...
for idx, row in df.iterrows():
    ...
    if any(k in name_lower for k in ["gmbh", "ag", "holding", "gesellschaft mit beschr", "transit-film"]) or nid in [
        626, 627, 628, 702, 312, 376, 503, 501, 505, 631, 662, 661, 663, 676, 700
    ]:
...
for row_idx, data in enumerate(nav_data):
    ...
    # Apply borders to all columns in the merged cells
...
def generate_data_sheet(ws_title, title_text, data_df, headers, is_gesamt=False):
    ...
    # Add formulas for Budget and Employees
    emp_col = -1
    bud_col = -1
    for idx, h in enumerate(headers):
    ...
    for col in ws.columns:
        ...
        # Apply padding and safety constraints
        ws.column_dimensions[col_letter].width = max(max_len + 3, 10)
        
    # Make sure we don't truncate names
